...
def flux(m, theta, ts, caustics = None, label = None, color = None, alpha = None, ls = None, lw = None):
    ...
    if caustics is not None:
        ...
        model.plot_caustics(color = color, s = 0.1, marker = 'o', n_points = 25000, label = label)
...
def broccoli(supset_states, subset_states, surrogate_supset_states, surrogate_subset_states, symbols, ranges, curves, event_params = None, name = '', dpi = 100):
    ...
    for yi in range(N_dim):
        for xi in range(yi):
            ...
            ax.scatter(binary_theta.scaled[xi+1], binary_theta.scaled[yi+1], color = 'tab:purple', alpha = 1.0, marker = "8", s = 75, linewidth = 1, zorder=10)
            ...
            if xi < n_dim and yi < n_dim:
                ...
                axt.scatter(x=single_theta.scaled[yi+1], y=single_theta.scaled[xi+1], color = 'tab:green', alpha = 1.0, marker = "8", s = 75, linewidth = 1, zorder=9)
    ...
    inset_ax.legend(fontsize = 14, handlelength=0.7, frameon = True, handletextpad=0.4)
    inset_ax.set_xlim([10, 20])
    inset_ax.set_ylim([1.1, 5.9])

    inset_ax.tick_params(which='both', top=True, right=True, direction='in', labelsize = 12)
    inset_ax.yaxis.set_minor_locator(ticker.AutoMinorLocator(n_m_ticks))
    inset_ax.xaxis.set_minor_locator(ticker.AutoMinorLocator(n_m_ticks))
