...
if __name__ == "__main__":
    ...
    for q in [0]:
        ...
        n_suite = 5
        n_epochs = 7200
        sn_base = 23#60 #(230-23)/2 + 23 #(lower = noisier).
        seed = 42 #42

        # Warm up parameters.
        fixed_warm_up_iterations = 100
        adaptive_warm_up_iterations = 400#/500 #400#975
        warm_up_repititions = 5
        ...
        model_parameters = [
            ...
            [0.5, 15, 0.1, 10, 0.01, 0.4, 60],  # 2
            [0.5, 15, 0.1, 10, 0.01, 0.5, 60],  # 3
            [0.5, 15, 0.1, 10, 0.01, 0.25, 60], # 4
            [0.5, 15, 0.1, 10, 0.01, 0.6, 60]]  # 5
        event_params = sampling.State(truth = model_parameters[n_suite])


        # Model index for parameters.
        model_types = [1, 1, 1, 1, 1, 1]
        model_type = model_types[n_suite]

        # Generate synthetic light curve.
        if model_type == 0:
            data = light_curve_simulation.synthetic_single(event_params, n_epochs, sn_base, seed)
        else: 
            dense_data = light_curve_simulation.synthetic_binary(event_params, n_epochs, sn_base, seed)
        ...
        #binary_sp = surrogate_posteriors.Surrogate_Posterior(1, light_curve_simulation.synthetic_binary(event_params, 7200, sn_base, seed).flux-1)
        binary_sp = surrogate_posteriors.Surrogate_Posterior(1, dense_data.flux-1)
        
        # reduce samples
        data = Data(dense_data.flux[0::10], dense_data.time[0::10], dense_data.err_flux[0::10])
        #data.flux=data.flux[0::10]
        #data.time=data.time[0::10]
        #data.err_flux=data.err_flux[0::10]
