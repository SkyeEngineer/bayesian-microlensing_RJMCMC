...
event_params = source.State(truth = model_parameters[suite_n])
...
if use_NN == True:
    ...
    [45, 0.2, 20], # 2
    [36, 0.1, 36]] # 4
    single_center = source.State(truth = np.array(single_centers[suite_n]))
    ...
    [36, 0.1, 36, 0.8, 0.25, 123]]   # 3
    binary_center = source.State(truth = np.array(binary_centers[suite_n]))
...
joint_model_chain = source.adapt_RJMH(Models, adapt_MH_warm_up, adapt_MH, initial_n, iterations, user_feedback)
